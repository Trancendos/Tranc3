...
export default function AdminPage() {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      {/* Warning */}
      <div className="flex items-start gap-3 bg-yellow-900/20 border border-yellow-700 rounded-lg p-4 mb-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-y-3 gap-x-6">
          {stats.map((s) => (
            <div key={s.label}>
              ...
              <p className={`text-sm font-medium ${s.color ?? 'text-gray-200'}`}>{s.value}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {SECTIONS.map((sec) => (
          <div
            key={sec.id}
            className={`bg-gray-900 border rounded-lg p-5 transition-colors ${sec.color} ${sec.href ? 'cursor-pointer' : 'opacity-60'}`}
            onClick={() => sec.href && window.open(sec.href, '_blank')}
          >
            <div className="flex items-start justify-between">
              ...
              {sec.href && <ChevronRight size={16} className="text-gray-600 flex-shrink-0 mt-1" />}
            </div>
            {!sec.href && (
