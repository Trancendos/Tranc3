...
class GoalManager:
    ...
    async def get_goal_summary(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            # CodeQL reads an Enum CLASS as non-iterable (its Python model does not
            # carry EnumMeta.__iter__). Adjudicated with the reason in
            # scripts/filter_codeql_alerts.py — an inline `# codeql[...]` comment
            # here did nothing, because the SARIF upload path does not honour them.
