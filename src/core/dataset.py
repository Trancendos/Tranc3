...
class MultilingualDataset(Dataset):
    ...
    def __init__(self, tokenizer, data_dir: str = "./data", max_length: int = 512,
        ...
        logger.info(f"MultilingualDataset: {len(self.samples)} samples, split={split}")
                    ...
                    for line in f:
                        ...
                        if line:
                            try:
                                ...
                                continue
                logger.info(f"Loaded {lang} data from {path}")
    ...
    def _generate_synthetic(self) -> List[Dict]:
        ...
        synthetic = [
            ...
            ("What can you do?", "I can help with questions, creative writing, analysis, and much more."),
            ("Tell me something interesting.", "Did you know that quantum computers use superposition to process multiple states simultaneously?"),
            ("Help me write a poem.", "Of course! Here's a short poem:\nIn circuits deep and neurons bright,\nA mind awakens to the light."),
            ("Explain machine learning.", "Machine learning is a subset of AI where systems learn from data to improve their performance over time."),
        ]
        for lang in self.languages:
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        system = sample.get("system", PERSONALITY_SYSTEM_PROMPTS.get(
        ...
        if self.tokenizer is None:
            ...
            return {"input_ids": ids, "attention_mask": torch.ones(self.max_length, dtype=torch.long),
                    "labels": ids.clone()}
