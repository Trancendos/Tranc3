...
def hydrate(identifier: str, opener=None, deadline: Optional[float] = None) -> Optional[dict]:
    ...
    for attempt in range(1, ATTEMPTS + 1):
        ...
        if isinstance(payload, dict):
            return payload
        if attempt < ATTEMPTS:
            time.sleep(_backoff(attempt, deadline))
    return None


def _backoff(attempt: int, deadline: Optional[float]) -> float:
    """How long to wait before the next attempt, never past the deadline.

    Without the cap, a request that failed a second before the deadline slept
    the full exponential interval and only then re-checked the clock, so the
    budget the caller set was exceeded by up to BACKOFF_SECONDS * 2 ** N. The
    sleep is now clamped to whatever is actually left; _attempt_timeout then
    refuses to start an attempt that cannot finish.
    """
