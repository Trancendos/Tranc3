...
#: Below this many seconds left, an attempt is not worth starting: it would
#: either be cut off mid-flight or overrun the caller's budget.
...
#: Cargo writes this header into every lockfile it generates, and the `version`
#: key has been present since the format was versioned. Either is enough to say
#: the file is a lockfile rather than a truncated fragment.
_LOCKFILE_MARKERS = ("@generated by Cargo", "[[package]]", "version = ")


def _looks_like_a_lockfile(text: str) -> bool:
    """Is this a lockfile with no registry packages, or a broken file?"""
    return any(marker in text for marker in _LOCKFILE_MARKERS)
...
def hydrate(identifier: str, opener=None, deadline: Optional[float] = None) -> Optional[dict]:
    ...
    for attempt in range(1, ATTEMPTS + 1):
        ...
        try:
            ...
            with send(url, timeout=timeout) as response:  # noqa: S310
...
def _attempt_timeout(deadline: Optional[float]) -> Optional[float]:
    """Seconds this attempt may take, or None when the deadline has passed.

    The census gives its whole run twenty minutes and kills the job at the
    limit. An OSV endpoint that accepts connections and then never answers
    would hold each request for the full per-call timeout, retry it twice, and
    do that for twelve surfaces — overrunning the budget entirely, and being
    killed reports nothing at all rather than reporting what could not be
    scanned.
    """
    if deadline is None:
...
def _post_batch(
    queries: Sequence[dict], opener=None, deadline: Optional[float] = None
) -> Optional[List[dict]]:
    ...
    for attempt in range(1, ATTEMPTS + 1):
        ...
        try:
            ...
            # nosemgrep: python.lang.security.audit.dynamic-urllib-use-detected.dynamic-urllib-use-detected
            with send(request, timeout=timeout) as response:  # noqa: S310
                payload = json.loads(response.read().decode("utf-8"))
        except (urllib.error.URLError, TimeoutError, ValueError, OSError):
            payload = None
...
def query_osv(
    ...
    for start in range(0, len(packages), BATCH_SIZE):
        ...
        results = _post_batch(queries, opener=opener, deadline=deadline)
        if results is None:
            return None
        if len(results) != len(chunk):
            # OSV answers positionally, so a short list means some packages went
            # unanswered. The first version of this stopped at whichever ran out
            # and called that "under-reporting is better than mis-attributing".
            # Both are true and both miss the point: an unanswered package
            # contributes no finding, and a surface with no findings reads as
            # CLEAN. Refusing the surface is the third option, and it is the one
    ...
    for identifier in sorted({identifier for _, _, identifier in hits}):
        record = hydrate(identifier, opener=opener, deadline=deadline)
        if record is None:
            return None
...
def scan_go_module(
    root: str, rel: str, opener=None, deadline: Optional[float] = None
) -> NativeSurface:
    ...
    results = query_osv(packages, "Go", opener=opener, deadline=deadline)
    if results is None:
        surface.errored = True
        surface.reason = (
            f"OSV unusable after up to {ATTEMPTS} attempts — unreachable, an "
            "incomplete batch response, an advisory that could not be hydrated, "
            "or the time budget ending before an attempt could start"
        )
        return surface
...
def scan_rust_crate(
    ...
    if not packages and not _looks_like_a_lockfile(text):
        # A truncated or malformed lockfile parses to zero packages, and zero
        # packages scans clean. A crate with genuinely no registry dependencies
        # is real and must still pass, so the test is on the FILE rather than
        # on the package count: a lockfile says what it is in its own header.
        surface.errored = True
        surface.reason = (
            "Cargo.lock yielded no registry packages and does not look like a "
            "lockfile — a malformed or truncated one parses to nothing, and "
            "nothing reads as clean"
        )
        return surface
    surface.queried = len(packages)
    results = query_osv(packages, "crates.io", opener=opener, deadline=deadline)
    if results is None:
        surface.errored = True
        surface.reason = (
            f"OSV unusable after up to {ATTEMPTS} attempts — unreachable, an "
...
def scan_all(
    ...
    for rel in discover_go_modules(root, excluded):
        out.append(scan_go_module(root, rel, opener=opener, deadline=deadline).to_dict())
    for rel in discover_rust_crates(root, excluded):
        out.append(scan_rust_crate(root, rel, opener=opener, deadline=deadline).to_dict())
    return out
