...
class WorkflowEventBus:
    ...
    def subscribe(self, event: str, callback: Callable) -> None:
        ...
        logger.debug("Subscribed to event '%s': %s", sanitize_for_log(event), sanitize_for_log(callback))
...
class WorkflowExecutor:
    ...
    async def execute(
        ...
        try:
            ...
            logger.info("Workflow '%s' completed in %.1fms.", sanitize_for_log(workflow.name), sanitize_for_log(state.elapsed_ms))

        except asyncio.CancelledError:
            state.status = "cancelled"
            state.finished_at = time.monotonic()
            logger.info("Execution %s cancelled via CancelledError.", sanitize_for_log(execution_id))
    ...
    async def _execute_layer(
        ...
        async def _run_node(node_id: str) -> None:
            ...
            if nc is None:
                ...
                logger.error("Node '%s' not found in workflow definition.", sanitize_for_log(node_id))
                return
            ...
            if result.success:
                ...
                logger.debug("Node '%s' completed in %.1fms.", sanitize_for_log(node_id), sanitize_for_log(result.duration_ms))
            else:
                state.node_statuses[node_id] = "failed"
                ...
                logger.warning("Node '%s' failed: %s", sanitize_for_log(node_id), sanitize_for_log(result.error))
