...
class InferenceWorker:
    ...
    async def _handle(self, job: JobSpec):
        async with self._sem:
            ...
            logger.info("Job %s [%s] done in %.1f ms", job.job_id, job.job_type, result.duration_ms)
    ...
    async def _do_emotion(self, p: dict) -> dict:
        ...
        if self._engine._bootstrap_mode or not self._engine._loaded:
            ...
            emotions = {
                "joy":     any(w in text_lower for w in ["happy", "great", "excellent", "wonderful", "love", "yay"]),
                "sadness": any(w in text_lower for w in ["sad", "unhappy", "terrible", "awful", "cry", "miss"]),
                "anger":   any(w in text_lower for w in ["angry", "furious", "hate", "rage", "frustrated"]),
                "fear":    any(w in text_lower for w in ["scared", "afraid", "fear", "worried", "anxious"]),
                "surprise":any(w in text_lower for w in ["wow", "amazing", "unexpected", "shocked", "unbelievable"]),
                "disgust": any(w in text_lower for w in ["disgusting", "horrible", "gross", "nasty", "repulsive"]),
            }
            ...
            return {"dominant": dominant, "scores": scores, "model": "tranc3-rule-based"}
    ...
    async def _do_tokenize(self, p: dict) -> dict:
        ...
        if action == "encode":
            ...
            return {"tokens": tokens, "ids": ids, "count": len(ids), "model": "tranc3-bpe"}
        else:
            ids  = p.get("ids", [])
    ...
    async def _do_personality(self, p: dict) -> dict:
        ...
        try:
            ...
            return {"profile": profile, "vector": vec.tolist(), "dims": len(vec), "model": "tranc3-personality"}
        except Exception:
            ...
            return {"profile": profile, "vector": vec, "dims": dim, "model": "tranc3-hash-personality"}

    async def _do_predict(self, p: dict) -> dict:
        text       = p.get("text", "")
        top_k      = p.get("top_k", 5)
        predict_type = p.get("predict_type", "next_token")
