...
PLATFORM_ENTITIES: Dict[str, LocationEntity] = {
    ...
    "Luminous": LocationEntity(
        ...
        agent_alpha=Agent(
            "Synapse",
            "Monitors global activity; triggers alerts for offline nodes.",
        ),
    ...
    "TateKing": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Director",
            "Coordinates timeline-as-code scripting from video data.",
        ),
    ...
    "Imaginarium": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Alchemist",
            "Translates product plans into multi-app design sequences.",
        ),
        ...
        bot_03=Bot(
            "Welder-Bot",
            "Links user input triggers in the UI directly to backend functions.",
        ),
    ...
    "The Digital Grid": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Flow-Weaver",
            "Weaves APIs, webhooks, and scripts into execution steps.",
        ),
        agent_beta=Agent(
            "Event-Broker",
            "Monitors webhooks, sending signals for automated triggers.",
        ),
    ...
    "The Lab": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Hounds",
            "Searches sandbox code for syntax errors and memory leaks.",
        ),
        agent_beta=Agent(
            "Syntax-Sage",
            "Reads active scripts, suggesting code optimization patterns.",
        ),
    ...
    "The Workshop": LocationEntity(
        ...
        agent_alpha=Agent(
            "Branch-Manager",
            "Tracks active code branches, conflicts, and pull requests.",
        ),
        agent_beta=Agent(
            "Merge-Master",
            "Safely merges code branches, guiding users through conflicts.",
        ),
    ...
    "The Chaos Party": LocationEntity(
        ...
        agent_alpha=Agent(
            "The March Hare",
            "Sends rapid mock inputs and payloads to stress-test systems.",
        ),
        agent_beta=Agent(
            "The Dormouse",
            "Sits silently in tests, measuring memory leaks/performance dips.",
        ),
        bot_01=Bot(
            "Teapot-Bot",
            "Spams server endpoints with massive requests to test load limits.",
        ),
        bot_02=Bot(
            "Pocket-Watch-Bot",
            "Tracks API response times during load spikes for latency alerts.",
        ),
        bot_03=Bot(
            "Sugar-Cube-Bot",
            "Generates messy mockup databases to test bad dataset handling.",
        ),
    ...
    "The Artifactory": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Librarian",
            "Catalogs compiled code assets, container images, and packages.",
        ),
        agent_beta=Agent(
            "The Archivist",
            "Packages ecosystem snapshots into safe, deployable restore files.",
        ),
        bot_01=Bot(
            "Packer-Bot",
            "Compiles software libraries and environments into container images.",
        ),
        bot_02=Bot("Unpacker-Bot", "Extracts container assets, mounting them in active servers."),
        bot_03=Bot(
            "Checksum-Bot",
            "Generates secure hashes to verify downloaded files are unmodified.",
        ),
    ...
    "API Marketplace": LocationEntity(
        ...
        agent_beta=Agent(
            "The Diplomat",
            "Handles external handshakes, authentications, and secure keys.",
        ),
    ...
    "Royal Bank of Arcadia": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Treasurer",
            "Monitors resource usage, scaling idle services to approach zero-cost.",
        ),
        agent_beta=Agent(
            "The Actuary",
            "Evaluates system runtime efficiency, mapping ROI metrics.",
        ),
        ...
        bot_04=Bot(
            "Receipt-Bot",
            "Generates transaction recaps, usage invoices, and expense charts.",
        ),
        worker_port=8013,
    ...
    "Arcadian Exchange": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Speculator",
            "Assesses server cost trends to buy bulk compute resources.",
        ),
        agent_beta=Agent(
            "The Trader",
            "Automates bidding on open compute marketplaces for affordability.",
        ),
        bot_01=Bot(
            "Bidder-Bot",
            "Submits buy requests on real-time server auctions for processes.",
        ),
        bot_02=Bot(
            "Asker-Bot",
            "Sets pricing rules for when external platforms buy Arcadian power.",
        ),
        bot_03=Bot(
            "Miner-Bot",
            "Utilizes idle GPU capacity to run calculations or generate assets.",
        ),
        bot_04=Bot(
            "Harvester-Bot",
            "Identifies and frees up neglected storage blocks across servers.",
        ),
        worker_port=8012,
    ...
    "The Observatory": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Watcher",
            "Scans monitoring logs in real-time for unusual anomalies/spikes.",
        ),
        agent_beta=Agent(
            "The Scribe",
            "Compresses long log files into searchable summary journals.",
        ),
        bot_01=Bot(
            "Log-Alpha-Bot",
            "Captures UI interactions, button clicks, and front-end errors.",
        ),
        bot_02=Bot(
            "Log-Beta-Bot",
            "Gathers background server signals, database calls, and backend tasks.",
        ),
        bot_03=Bot(
            "Tracer-Bot",
            "Tracks data paths across multiple servers to isolate bottlenecks.",
        ),
        bot_04=Bot(
            "Timestamp-Bot",
            "Applies high-precision UTC marks to every event for accuracy.",
        ),
        worker_port=8007,
    ...
    "The Library": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Curator",
            "Identifies duplicated wiki pages and flags outdated articles.",
        ),
        agent_beta=Agent("The Indexer", "Adds searchable tags and conceptual links to wiki pages."),
        bot_01=Bot(
            "Page-Bot",
            "Processes text inputs, rendering clean wiki documents in markdown.",
        ),
        bot_02=Bot("Bookmark-Bot", "Logs user favourite files and recent reading history."),
        bot_03=Bot(
            "Spine-Bot",
            "Ensures all internal page links work, keeping documents connected.",
        ),
    ...
    "The Academy": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Tutor",
            "Modifies materials and guides to match user progress/skill.",
        ),
        agent_beta=Agent(
            "The Proctor",
            "Evaluates practice coding exercises and logs test scores.",
        ),
        bot_01=Bot(
            "Chalk-Bot",
            "Projects visual charts and interactive terminal sandboxes in the UI.",
        ),
        bot_02=Bot("Board-Bot", "Manages course paths, student lists, and syllabus structures."),
        bot_03=Bot(
            "Eraser-Bot",
            "Resets coding sandboxes, removing trial code for the next lesson.",
        ),
    ...
    "DocUtari": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Filer",
            "Places files in structured folders, ensuring quick retrieval.",
        ),
        ...
        bot_02=Bot(
            "Stapler-Bot",
            "Bundles related drafts, spreadsheets, and pictures into packets.",
        ),
        bot_03=Bot(
            "Folder-Bot",
            "Handles privacy and permission rules on individual files/directories.",
        ),
        bot_04=Bot(
            "Shredder-Bot",
    ...
    "The Basement": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Undertaker",
            "Finds stale databases, archiving them in cold storage.",
        ),
        agent_beta=Agent(
            "The Miner",
            "Searches deep archive catalogs, pulling up requested documents.",
        ),
        bot_01=Bot(
            "Compressor-Bot",
            "Runs file compression routines to keep cold storage costs low.",
        ),
        ...
        bot_04=Bot(
            "Mothball-Bot",
            "Encrypts and locks retired legacy versions of platform software.",
        ),
        worker_port=None,
    ...
    "The Spark": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Matchmaker",
            "Matches multi-node requests with the correct skilled AI/Agent.",
        ),
        agent_beta=Agent(
            "The Router",
            "Re-routes service queries if a designated AI has high-load delays.",
        ),
        ...
        bot_03=Bot(
            "Linker-Bot",
            "Establishes secure channels between seeking and assisting nodes.",
        ),
        bot_04=Bot(
            "Pinger-Bot",
    ...
    "Infinity": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Gatekeeper",
            "Checks incoming user logins, issuing secure, temporary keys.",
        ),
        agent_beta=Agent(
            "The Bouncer",
            "Monitors login origins and activities, blocking suspicious IPs.",
        ),
        bot_01=Bot(
            "Token-Minter-Bot",
            "Generates secure, time-limited tokens for node-crossing users.",
        ),
        bot_02=Bot(
            "Auth-Check-Bot",
            "Verifies active user permissions before unlocking private features.",
        ),
        bot_03=Bot(
            "Key-Gen-Bot",
            "Handles local encryption keys to authorize offline applications.",
        ),
    ...
    "The Void": LocationEntity(
        ...
        agent_alpha=Agent(
            "Crypt-Keeper",
            "Coordinates zero-knowledge DB access; splits/protects keys.",
        ),
        agent_beta=Agent(
            "The Silencer",
            "Sanitizes outbound streams so sensitive data avoids general logs.",
        ),
        bot_01=Bot("Hash-Bot", "Converts passwords and secrets into secure cryptographic strings."),
        bot_02=Bot(
            "Salt-Bot",
            "Adds randomized padding to password strings to prevent dictionary attacks.",
        ),
        bot_03=Bot(
            "Cipher-Bot",
            "Runs real-time encryption and decryption on active secure files.",
        ),
        bot_04=Bot(
            "Padlock-Bot",
            "Instantly locks sensitive structures if a local breach is suspected.",
        ),
        worker_port=8024,
    ...
    "The Lighthouse": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Minter",
            "Mints unique cryptographic identifier tokens for all new content.",
        ),
        agent_beta=Agent(
            "The Stamper",
            "Attaches verified, tamper-evident digital metadata to packets.",
        ),
        bot_01=Bot(
            "Seal-Bot",
            "Locks system-state snapshots, preventing unauthorized modifications.",
        ),
        bot_02=Bot(
            "Wax-Bot",
            "Generates temporary, single-use visual watermarks for digital assets.",
        ),
        bot_03=Bot(
            "Signet-Bot",
            "Validates credentials, signing certificates for structural operations.",
        ),
        bot_04=Bot(
            "Seal-Stamp-Bot",
            "Applies file-system metadata to register the exact creation details.",
        ),
        worker_port=8015,
    ...
    "The Warp Tunnel": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Warden",
            "Oversees integrity scans for mutated or corrupted system files.",
        ),
        agent_beta=Agent(
            "The Inspector",
            "Compares active database hashes against secure Lighthouse standards.",
        ),
        bot_01=Bot(
            "Scan-Bot",
            "Performs background sweeps on directories, reading cryptographic stamps.",
        ),
        bot_02=Bot(
            "Sniffer-Bot",
            "Analyzes transport packets for corrupted signatures/manipulations.",
        ),
        bot_03=Bot(
            "Beam-Bot",
            "Isolates threatened memory spaces, cutting off surrounding connections.",
        ),
        bot_04=Bot(
            "Portal-Bot",
            "Safely moves compromised file layers directly into the secure Ice Box.",
        ),
        worker_port=None,
    ...
    "Cryptex": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Shield",
            "Configures dynamic firewall rules, blocking network threats live.",
        ),
        agent_beta=Agent(
            "The Spear",
            "Automatically performs pen-testing against internal defenses.",
        ),
        bot_01=Bot(
            "Blocker-Bot",
            "Blacklists malicious IP ranges, halting DDoS attacks at the gateway.",
        ),
        bot_02=Bot("Trace-Bot", "Traces malicious attacks back to origin networks for reporting."),
        bot_03=Bot(
            "Patcher-Bot",
            "Applies emergency system patches to vulnerable software layers.",
        ),
        bot_04=Bot(
            "Honeypot-Bot",
    ...
    "The Ice Box": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Jailer",
            "Manages secure quarantine zones, keeping malicious payloads isolated.",
        ),
        agent_beta=Agent(
            "The Interrogator",
            "Triggers/monitors quarantine code execution to document behaviors.",
        ),
        bot_01=Bot(
            "Frostbite-Bot",
            "Halts execution threads when sandbox boundaries are breached.",
        ),
        bot_02=Bot(
            "Icicle-Bot",
            "Freezes dynamic processes to snapshot active RAM and memory spaces.",
        ),
        bot_03=Bot(
            "Glacier-Bot",
    ...
    "Warp Radio": LocationEntity(
        ...
        agent_alpha=Agent(
            "The DJ",
            "Curates spatial music playlists/soundscapes to match user activities.",
        ),
        agent_beta=Agent(
            "The Maestro",
            "Dynamically balances system alert volumes with external platforms.",
        ),
        bot_01=Bot(
            "Play-Bot",
            "Connects/streams music data directly from Spotify, Apple, and Amazon.",
        ),
        bot_02=Bot(
            "Pause-Bot",
            "Holds audio states and syncs current track metrics across devices.",
        ),
        bot_03=Bot(
            "Skip-Bot",
            "Fetches adjacent track metadata, pre-buffering streams to stop latency.",
        ),
        bot_04=Bot(
            "Volume-Bot",
    ...
    "Section 7": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Spy",
            "Gathers sentiment data from public channels to gauge market trends.",
        ),
        agent_beta=Agent(
            "The Oracle",
            "Converts intelligence records into structured development blueprints.",
        ),
        bot_01=Bot(
            "Scraper-Bot",
            "Pulls text data from developer channels, social spaces, and trackers.",
        ),
        bot_02=Bot(
            "Parser-Bot",
            "Sanitizes and categorizes scraped data, cleaning up format issues.",
        ),
        bot_03=Bot(
            "Crawler-Bot",
            "Dispatches web agents to identify relevant API and tech changes.",
        ),
        bot_04=Bot(
            "Whisper-Bot",
            "Delivers summarized threat and trend alerts directly to strategic hubs.",
        ),
        worker_port=8027,
    ...
    "The Citadel": LocationEntity(
        ...
        agent_alpha=Agent(
            "The General",
            "Directs high-level development priorities, adjusting assignments.",
        ),
        agent_beta=Agent(
            "The Tactician",
            "Re-allocates team structures to meet immediate design objectives.",
        ),
        bot_01=Bot(
            "Map-Bot",
            "Projects system metrics on command dashboards, visualizing node relations.",
        ),
        ...
        bot_04=Bot(
            "Radio-Bot",
            "Broadcasts executive platform priorities directly to low-tier Agents.",
        ),
        worker_port=None,
        worker_path="deploy/",
    ),
    "Think Tank": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Professor",
            "Simulates untested programmatic changes to assess system impacts.",
        ),
        agent_beta=Agent(
            "The Visionary",
            "Suggests structural updates and feature builds based on telemetry.",
        ),
        bot_01=Bot(
            "Beaker-Bot",
            "Handles lightweight, isolated experiment runtimes to test novel ideas.",
        ),
    ...
    "ChronosSphere / ArcStream": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Timekeeper",
            "Rearranges task backlogs and priorities to prevent bottlenecks.",
        ),
        agent_beta=Agent(
            "The Time-Weaver",
            "Translates timeline parameters into interactive visual Gantt views.",
        ),
        bot_01=Bot(
            "Tick-Bot",
        ...
        bot_02=Bot(
            "Tock-Bot",
            "Evaluates run times, flagging processes that run over temporal parameters.",
        ),
        bot_03=Bot(
            "Pendulum-Bot",
    ...
    "DevOcity": LocationEntity(
        ...
        agent_beta=Agent(
            "The Dispatcher",
            "Launches automated server scaling, optimizing system allocations.",
        ),
    ...
    "Tranquility": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Guide",
            "Screens user stress metrics, suggesting wellbeing sub-nodes for relief.",
        ),
        agent_beta=Agent(
            "The Healer",
        ...
        bot_01=Bot(
            "Breath-Bot",
            "Plays pacing animations to guide calm, measured breathing patterns.",
        ),
        bot_02=Bot(
            "Pulse-Bot",
    ...
    "tAimra": LocationEntity(
        ...
        agent_alpha=Agent(
            "The Shadow",
            "Mirrors daily habits to anticipate system and scheduling preferences.",
        ),
        agent_beta=Agent(
            "The Scheduler",
        ...
        bot_02=Bot(
            "Fetch-Bot",
            "Parses upcoming tasks to flag heavy scheduling days early for the user.",
        ),
        bot_03=Bot(
            "Nudge-Bot",
    ...
    "VRAR3D": LocationEntity(
        ...
        bot_02=Bot(
            "Track-Bot",
            "Translates head and hand actions into virtual movement inside scenes.",
        ),
        bot_03=Bot(
            "Haptic-Bot",
