    ...
    async def emit_lifecycle(
        ...
        for listener in self._listeners.get(key, []):
            try:
                ...
                logger.error(f"Error in lifecycle listener for event {event}: {e}")  # listeners should not crash the emitter
        ...
        for listener in self._any_listeners:
            try:
                ...
                logger.error(f"Error in any-lifecycle listener for event {event}: {e}")
    ...
    def emit_lifecycle_sync(
        ...
        for listener in self._listeners.get(key, []):
            try:
                ...
                logger.error(f"Error in lifecycle listener for event {event}: {e}")
