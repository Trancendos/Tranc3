...
REQUIRED_HEADER_PATTERN = re.compile(
    r"#\s*FID:\s*TRANC3-[A-Z]+-\d+", re.MULTILINE
)
...
class AbuseDetector:
    ...
    def check_ip(self, ip: str) -> Dict:
        ...
        if record.blocked_until > now:
            return {"allowed": False, "reason": "IP temporarily blocked", "retry_after": int(record.blocked_until - now)}
        ...
        if recent > self.SCRAPING_THRESHOLD:
            ...
            return {"allowed": False, "reason": "Rate abuse detected — IP blocked for 1 hour"}
    ...
    def check_message(self, message: str, user_id: str) -> Dict:
        ...
        if violations:
            ...
            return {"allowed": False, "violations": violations, "action": "message_blocked"}
