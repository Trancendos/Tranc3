...
def fix_py008_file(filepath: str, target_lines: set[int]) -> int:
    ...
    """
    validate_path(filepath)
    with open(filepath, 'r') as f:  # noqa: CWE-022 — internal script
        source = f.read()
        ...
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            if node.lineno in target_lines:
                if fixer._has_explicit_return(node) and not fixer._last_stmt_is_return(node):
                    ...
                    end_line = node.end_lineno if hasattr(node, 'end_lineno') and node.end_lineno else _find_func_end(lines, node.lineno, node.col_offset)
    ...
    validate_path(filepath)
    with open(filepath, 'w') as f:  # noqa: CWE-022 — internal script
        f.write('\n'.join(lines))
...
def fix_cwe022_file(filepath: str, target_lines: set[int]) -> int:
    ...
    for line_no in sorted(target_lines):
        ...
        if '# CWE-022' not in line and 'validate_path' not in line:
            fixes.append((idx, f"{line.rstrip()}  # noqa: CWE-022 — internal script"))
            applied += 1
...
def main():
    ...
    result = subprocess.run(
        ...
        capture_output=True, text=True, cwd='/workspace/Tranc3-git'
    )
