    ...
    for line_no in sorted(target_lines):
        ...
        if open_match:
            ...
            if "validate_path" not in source:
                # Add import at top
                _add_import(lines, "from Dimensionals.path_validation import validate_path")
                source = "\n".join(lines)
...
def fix_cwe209_file(filepath: str, target_lines: set[int]) -> int:
    ...
    if applied and needs_import:
        _add_import(lines, "from Dimensionals.error_handlers import safe_error_detail")
...
def main():
    ...
    result = subprocess.run(
        [sys.executable, "-m", "Dimensionals.security_automation", "scan", "--format", "text", "."],
        capture_output=True,
        text=True,
