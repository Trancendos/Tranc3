...
KNOWN_COUPLED: dict[str, str] = {
    ...
    "_library_router": (
        ...
        "forwards newly-created articles for durability, narrowed (2026-08-08 "
        "review) to PUBLIC/INTERNAL only — CONFIDENTIAL/RESTRICTED/TOP_SECRET "
        "are never sent, regardless of reachability, since 'authenticated "
        "platform user' and 'holds the shared secret' are different kinds of "
        "trust even where routes.py's own _can_read() treats them the same. "
        "Also gated on src.security.log_redactor.contains_pii() independent "
        "of classification, since a PUBLIC/INTERNAL article can still "
        "contain personal data the label doesn't capture. Deletes propagate "
        "too (Library.delete() -> forward_delete() -> the worker's new "
        "DELETE /library/documents/{doc_id}), so a source deletion doesn't "
        "leave an orphaned durable copy. Fail-open throughout: a forward or "
        "delete failure never blocks the in-process write."
    ),
    "_turingshub_router": (
