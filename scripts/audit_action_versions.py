...
def parse_uses(value: str) -> dict | None:
    ...
    return {"kind": "github", "owner": owner, "repo": repo,
            "subpath": subpath, "ref": ref, "sha": sha.lower()}
...
def main() -> int:
    ...
    for wf_dir in WORKFLOW_DIRS:
        ...
        for path in sorted(base.rglob("*.yml")) + sorted(base.rglob("*.yaml")):
            ...
            for idx, line in enumerate(lines, start=1):
                ...
                if err:
                    findings.append({"file": rel, "line": idx, "ref": parsed["ref"],
                                     "pinned": parsed["sha"], "status": "unknown",
                                     "detail": err})
                    continue
                if parsed["sha"] == commit:
                    ...
                    status = "outdated"
                findings.append({"file": rel, "line": idx, "ref": parsed["ref"],
                                 "pinned": parsed["sha"], "latest_tag": tag,
                                 "latest_commit": commit, "status": status})

    outdated = [f for f in findings if f["status"] == "outdated"]
    unknown = [f for f in findings if f["status"] == "unknown"]

    for f in outdated:
        msg = (f"Outdated action {f['ref']} pinned at {f['pinned'][:12]} "
    ...
    report = {"total_references": len(findings) + len(external),
    ...
    md = [f"# GitHub Action Version Audit",
          "",
    ...
    if outdated:
        for f in outdated:
            md.append(f"- `{f['ref']}` in `{f['file']}:{f['line']}` — "
