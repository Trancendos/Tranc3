...
def _summary(surface: str, alert: dict[str, Any]) -> str:
    ...
    if surface == "code_scanning":
        return str(_get_path(alert, "rule", "description") or _get_path(alert, "rule", "id") or "code finding")
    return str(alert.get("secret_type_display_name") or alert.get("secret_type") or "secret finding")
...
def build_report(repo: str, results: list[SurfaceResult], findings: list[dict[str, Any]]) -> dict[str, Any]:
    return {
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    for name, surface in report["surfaces"].items():
        lines.append(f"| {name} | {surface['status']} | {surface['count']} | {surface['error'] or ''} |")
    lines.extend(["", "## Highest-priority findings", ""])
    for finding in report["findings"][:25]:
        lines.append(f"- **{finding['priority']} / {finding['severity']}** `{finding['id']}` — {finding['summary']}")
...
def main() -> int:
    ...
    (args.output_dir / "security_backlog.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
