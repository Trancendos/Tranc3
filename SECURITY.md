...
|---------|---------|--------------------|-------------------|
| `ecdsa` (transitive via `python-jose`, hard-required — `Requires-Dist: ecdsa!=0.15`, no compatible-range override exists) | CVE-2024-23342 / PYSEC-2026-1325 — Minerva timing side-channel in `SigningKey.sign_digest()` / ECDH | Maintainers have stated side-channel attacks are out of scope; no planned fix (`fix_versions: []`). This app's JWT usage (`src/auth/tokens.py`, `src/auth/facade.py`) is HS256-only — the vulnerable ECDSA sign/ECDH paths are never invoked. | If ES256/ES384/ES512 JWT support is ever added |
