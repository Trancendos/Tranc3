...
| ~~`mcp==1.23.3`~~ **RESOLVED 2026-09-11** (transitive via `semgrep`) | CVE-2026-52869/52870/59950 (PYSEC-2026-3481/3482/3483) — session-hijacking / missing Host-Origin validation in `mcp`'s SSE, WebSocket, and experimental-tasks *server* transports | **Resolution:** semgrep moved its pin from `mcp==1.23.3` (6 OSV advisories) to `mcp==1.29.0` (0 advisories) in releases 1.173.0+. The floating lower bound `semgrep>=1.60` in `requirements-oss.txt` resolves to current semgrep releases and therefore already takes `mcp==1.29.0`. The three PYSEC advisories no longer apply, and no override was needed to close them. (Originally accepted because overriding the exact pin broke pip's resolver; semgrep never starts an `mcp` server, so the vulnerable code was never reached.) | Re-measure before re-accepting if pip-audit reports an `mcp` advisory again |
