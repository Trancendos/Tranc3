...
## 2024-05-18 - Missing ARIA attributes pattern in custom tabs
**Learning:** Found an accessibility issue pattern: custom tab components (e.g. `VaultPage.tsx`) lacked semantic ARIA attributes, failing screen reader compatibility tests.
**Action:** Always ensure custom tab components use `role="tablist"` for the container, `role="tab"` for the toggle buttons, `aria-selected` to indicate active state visually and semantically, and `role="tabpanel"` for the content panes. Link tabs and panels using `id`, `aria-controls`, and `aria-labelledby`.
