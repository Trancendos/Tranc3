...
|-------|---|------------------|-----------------|
| **A. P0 code & automation** | **~98%** | Gate tests, metrics, Docker build context, compose validate | **Met** (≥85%) |
...
| Issue | Impact |
|-------|--------|
| Prometheus `/metrics` on P0 workers | **Fixed** — `mount_prometheus_endpoint` + repo-root Dockerfiles |
| P0 worker images missing `src/` in Docker | **Fixed** — `scripts/ensure_p0_worker_images.py` |
| `AUDIT_SIGNING_KEY` | Generated by `make generate-prod-env`; set in production |
