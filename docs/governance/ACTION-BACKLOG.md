...
| Story | Location | Status | Pts | Sized because | Source |
|---|---|---|---|---|---|
...
| Threat intelligence | Cryptex | Partial | 3 | baseline (+1); evidence/attestation register, not a code change (+2) | `docs/compliance/ISO27001_SOA.md:29` |
...
| Response to information security incidents | The Ice Box | Planned | 3 | baseline (+1); evidence/attestation register, not a code change (+2) | `docs/compliance/ISO27001_SOA.md:48` |
...
| Information security awareness, education, and training | The Academy | Planned | 3 | baseline (+1); evidence/attestation register, not a code change (+2) | `docs/compliance/ISO27001_SOA.md:69` |
...
| Responsibilities after termination | Infinity | Planned | 3 | baseline (+1); evidence/attestation register, not a code change (+2) | `docs/compliance/ISO27001_SOA.md:71` |
...
| The Ice Box / Warp Tunnel planned (WAV-001) | The Ice Box | PLANNED | 3 | baseline (+1); evidence/attestation register, not a code change (+2) | `docs/defstan/COMPLIANCE_REGISTER.md:53` |
...
| Story | Location | Status | Pts | Sized because | Source |
|---|---|---|---|---|---|
| Issue `PENPOT_TOKEN` into The Void | Fabulousa | Needs owner | 3 | baseline (+1); status `Needs owner` — impeded, not merely unstarted (+2) | `docs/architecture/FORENSIC-ASSESSMENT-CREATIVE-DELIVERY.md:201` |
| Fabulousa serves `tokens.ts`, components and widgets | Fabulousa | Open | 1 | baseline (+1) | `docs/architecture/FORENSIC-ASSESSMENT-CREATIVE-DELIVERY.md:202` |
...
| Chaos Party and Cryptex file PLM evidence automatically | Cryptex | Open | 1 | baseline (+1) | `docs/architecture/FORENSIC-ASSESSMENT-CREATIVE-DELIVERY.md:204` |
...
| Deploy TranceFlow / TateKing / Warp Radio `worker.py | TranceFlow | Open | 1 | baseline (+1) | `docs/architecture/FORENSIC-ASSESSMENT-CREATIVE-DELIVERY.md:207` |
...
| Story | Location | Status | Pts | Sized because | Source |
|---|---|---|---|---|---|
| Workflow and pipeline execution routes through The Digital Grid | The Digital Grid | partial | 1 | baseline (+1) | `docs/governance/LOCATION-FLOW-CONTRACT.md:91` |
| The Workshop holds repositories and mirrors to GitHub, GitLab and Bitbucket | The Workshop | partial | 1 | baseline (+1) | `docs/governance/LOCATION-FLOW-CONTRACT.md:93` |
| All financial regulation routes through Royal Bank of Arcadia | Arcadia | partial | 1 | baseline (+1) | `docs/governance/LOCATION-FLOW-CONTRACT.md:94` |
| All testing routes through The Chaos Party | The Chaos Party | partial | 1 | baseline (+1) | `docs/governance/LOCATION-FLOW-CONTRACT.md:103` |
...
| Rust crypto layer for The Void (PyO3 FFI) | The Void | Planned | 1 | baseline (+1) | `wiki-content/Historical-FORENSIC_REPORT_2026_05_28.md:280` |
...
| The Lab service foundation | The Lab | Planned | 1 | baseline (+1) | `wiki-content/Historical-FORENSIC_REPORT_2026_05_28.md:284` |
...
| Full Section 7 with automated scheduling | Section 7 | Planned | 1 | baseline (+1) | `wiki-content/Historical-FORENSIC_REPORT_2026_05_28.md:291` |
...
| Imaginarium service foundation | Imaginarium | Planned | 1 | baseline (+1) | `wiki-content/Historical-FORENSIC_REPORT_2026_05_28.md:293` |
...
| the `api` service in `docker-compose.development.yml` runs the monolith router — the standalone `cryptex` w… | Cryptex | Partial | 1 | baseline (+1) | `docs/services/cryptex/README.md:167` |
| same monolith router via `api` in `docker-compose.uat.yml` — the standalone `cryptex` worker is **not** in … | Cryptex | Partial | 1 | baseline (+1) | `docs/services/cryptex/README.md:168` |
| the `api` service in `docker-compose.development.yml` runs the monolith router — the standalone `devocity` … | DevOcity | Partial | 1 | baseline (+1) | `docs/services/devocity/README.md:154` |
| same monolith router via `api` in `docker-compose.uat.yml` — the standalone `devocity` worker is **not** in… | DevOcity | Partial | 1 | baseline (+1) | `docs/services/devocity/README.md:155` |
...
| the `api` service in `docker-compose.development.yml` runs the monolith router — the standalone `resonate` … | Resonate | Partial | 1 | baseline (+1) | `docs/services/resonate/README.md:142` |
| same monolith router via `api` in `docker-compose.uat.yml` — the standalone `resonate` worker is **not** in… | Resonate | Partial | 1 | baseline (+1) | `docs/services/resonate/README.md:143` |
| the `api` service in `docker-compose.development.yml` runs the monolith router — the standalone `taimra` wo… | tAimra | Partial | 1 | baseline (+1) | `docs/services/taimra/README.md:144` |
| same monolith router via `api` in `docker-compose.uat.yml` — the standalone `taimra` worker is **not** in t… | tAimra | Partial | 1 | baseline (+1) | `docs/services/taimra/README.md:145` |
...
| the `api` service in `docker-compose.development.yml` runs the monolith router — the standalone `tranquilit… | Tranquility | Partial | 1 | baseline (+1) | `docs/services/tranquility/README.md:139` |
| same monolith router via `api` in `docker-compose.uat.yml` — the standalone `tranquility` worker is **not**… | Tranquility | Partial | 1 | baseline (+1) | `docs/services/tranquility/README.md:140` |
...
| the `api` service in `docker-compose.development.yml` runs the monolith router — the standalone `vrar3d` wo… | VRAR3D | Partial | 1 | baseline (+1) | `docs/services/vrar3d/README.md:149` |
| same monolith router via `api` in `docker-compose.uat.yml` — the standalone `vrar3d` worker is **not** in t… | VRAR3D | Partial | 1 | baseline (+1) | `docs/services/vrar3d/README.md:150` |
