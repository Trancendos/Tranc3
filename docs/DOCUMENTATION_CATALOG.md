...
| Runtime Library review and publication | `docs/DOCUMENT_LIFECYCLE.md` | explanation | Chief Knowledge Officer (The Library) | The Workshop -> The Town Hall; intake: library-reviewed; destinations: The Library durable store, Search and RAG consumers | `src/library/knowledge_base.py`, `src/library/routes.py`, `src/library/bridge.py`, `src/townhall/itsm.py`, `src/townhall/itsm_routes.py`, `src/event_bus/wiring.py`, `src/observability/library_pipeline.py`, `src/basement/promotion.py`, `src/research/section7.py`, `src/models/knowledge.py` |
| Asset lifecycle and Artifactory custody | `docs/ASSET_LIFECYCLE.md` | reference | Platform Security and Supply Chain (DocUtari) | The Artifactory -> The Town Hall; intake: manual-only; destinations: The Artifactory custody ledger, DocUtari durable store | `workers/artifactory-service`, `workers/files-service`, `workers/ice-box-service`, `config/estate/flow_contract.yaml` |
