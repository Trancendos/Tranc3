...
**Role.** MCP server — AI tool registry, JSON-RPC 2.0 over HTTP/SSE (Norman Hawkins is The Spark's **Prime**, Tier 2 — not its Lead AI)
...
| Readiness | 7/10 | status ✅ in CLAUDE.md (+3); code path `src/mcp/` exists on disk (+2); three or more Python files present (+2) |
