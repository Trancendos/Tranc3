...
| Code path | `src/resonate/` ✅ on disk | filesystem |
...
- Runs in-process under `api.py` (path `src/resonate/`), so it *may*
...
| Readiness | 9/10 | status ✅ in CLAUDE.md (+3); code path `src/resonate/` exists on disk (+2); 3 Python files present (+2); compose service `resonate` defined (+2) |
