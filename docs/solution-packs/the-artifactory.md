---
title: "Solution Pack — The Artifactory"
category: Reference
...
| Code path | `src/artifactory/` ✅ on disk | filesystem |
...
- Runs in-process under `api.py` (path `src/artifactory/`), so it *may*
...
flowchart LR
    C[Client] --> A[api.py]
    A --> S[The Artifactory<br/>no port]
...
- As an operator, The Artifactory has a `docker-compose.production.yml` service with an explicit `PORT`.
...
| Readiness | 7/10 | status ✅ in CLAUDE.md (+3); code path `src/artifactory/` exists on disk (+2); 3 Python files present (+2) |
...
- `src/artifactory/` — implementation
