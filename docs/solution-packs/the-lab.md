...
| Code path | `src/lab/` ✅ on disk | filesystem |
...
- Runs in-process under `api.py` (path `src/lab/`), so it *may*
...
| Readiness | 9/10 | status ✅ in CLAUDE.md (+3); code path `src/lab/` exists on disk (+2); 3 Python files present (+2); compose service `the-lab` defined (+2) |
