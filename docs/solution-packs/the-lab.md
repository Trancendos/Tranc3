...
- **ROUTING DEFECT — `/the-lab` has no `stripprefix` and this worker
  does not serve the prefixed path.** Verified against its own source: every
  route it registers sits below `/`, not below `/the-lab`, so Traefik
  forwards `/the-lab/x` unchanged and the worker 404s on all of them.
...
```
1. Request arrives  →  Traefik forwards /the-lab unchanged (no stripprefix)
...
```yaml
  the-lab:
    ...
    ports: [ "8055:8055" ]
    labels:
      - "traefik.http.routers.the-lab.rule=Host(`the-lab.trancendos.com`) && PathPrefix(`/the-lab`)"
```
...
- As a client, requests to `/the-lab` reach a route the worker actually serves — today they do not, and this epic is that fix.
- As an implementer, EITHER a stripprefix middleware for `/the-lab` is added to the compose labels, OR the worker's router is given the prefix. One of the two, not neither.
