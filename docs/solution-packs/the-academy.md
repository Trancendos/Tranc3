---
title: "Solution Pack — The Academy"
category: Reference
...
| Code path | `src/academy/` ⚠️ not found | filesystem |
...
- Runs in-process under `api.py` (path `src/academy/`), so it *may*
...
### Epic 1 — Stand up the service

- As an operator, I can reach `GET /health` on The Academy so the compose healthcheck passes.
- As a developer, `src/academy/` exists with a FastAPI app and Dockerfile.
- As a reviewer, the worker imports nothing from `src/` (build-context constraint).

### Epic 2 — Verify routing end to end
...
### Epic 3 — Implement the abilities
...
|---|---|---|
| Registered path `src/academy/` does not exist | filesystem check | Registry claims an implementation that is not there |
...
| Readiness | 5/10 | status ✅ in CLAUDE.md (+3); compose service `the-academy` defined (+2) |
