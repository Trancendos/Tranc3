...
| Code path | `src/taimra/` ✅ on disk | filesystem |
...
- Runs in-process under `api.py` (path `src/taimra/`), so it *may*
...
| Readiness | 9/10 | status ✅ in CLAUDE.md (+3); code path `src/taimra/` exists on disk (+2); 3 Python files present (+2); compose service `taimra` defined (+2) |
