...
flowchart LR
    C[Client] --> T[Traefik]
    T -->|/the-dutchy| S[Section 7<br/>8057]
...
| Readiness | 8/10 | status ✅ in CLAUDE.md (+3); code path `workers/the-dutchy/` exists on disk (+2); at least one Python file present (+1); compose service `the-dutchy` defined (+2) |
