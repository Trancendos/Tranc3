...
| Field | Value | Source |
|---|---|---|
| Location | Think Tank | `src/entities/platform.py` |
| Product ID | `PID-TNK` | `_assign_ids()` |
| Lead AI | Trancendos (`AID-TNK-01`) | `src/entities/platform.py` |
| Base model tier | T2ance | `get_orchestration_tier()` |
| Job Description | Head of Research & Development | `docs/governance/LOCATION-FUNCTIONS.md` |
| Pillar | DevOps | `Pillar` enum |
| Reports to Prime(s) | Cornelius MacIntyre | `primes` |
| Status | ✅ In repo | `CLAUDE.md` service table |
| Code path | `src/quantum/` ✅ on disk | filesystem |
| Port | _unassigned_ | compose / `worker_port` |

**Role.** R&D centre — solutions & forefront technologies

## 2. What it does — DERIVED

**Primary function.** R&D Centre
...
- *Online:* Live research collaboration; active simulation running.
- *Offline:* Offline theoretical drafting; localized research models.
...
- Runs in-process under `api.py` (path `src/quantum/`), so it *may*
...
flowchart LR
    C[Client] --> A[api.py]
    A --> S[Think Tank<br/>no port]
    S --> DB[(SQLite<br/>own file)]
    S -.reports.-> P[Cornelius MacIntyre]
    S --> AA[The Professor]
    S --> AB[The Visionary]
    AA -.-> BeakerBot[Beaker-Bot]
    AA -.-> BunsenBot[Bunsen-Bot]
    AA -.-> PipetteBot[Pipette-Bot]
    AA -.-> PetriBot[Petri-Bot]
...
| Domain | The Professor + The Visionary | the two Agents below |
| Automation | Beaker-Bot, Bunsen-Bot, Pipette-Bot, Petri-Bot | the four Bots below |
...
2. The Professor — Simulates untested programmatic changes to assess system impacts.
3. The Visionary — Suggests structural updates and feature builds based on telemetry.
4. Bots fire: Beaker-Bot, Bunsen-Bot, Pipette-Bot, Petri-Bot
...
```sql
CREATE TABLE IF NOT EXISTS think_tank_records (
...
CREATE INDEX IF NOT EXISTS idx_think_tank_state ON think_tank_records(state);

CREATE TABLE IF NOT EXISTS think_tank_audit (
    id           INTEGER PRIMARY KEY AUTOINCREMENT,
...
|---|---|---|
| `SID-TNK-01` | The Professor | Simulates untested programmatic changes to assess system impacts. |
| `SID-TNK-02` | The Visionary | Suggests structural updates and feature builds based on telemetry. |

**Bots (Tier 5)**

| NID | Bot | Responsibility |
|---|---|---|
| `NID-TNK-01` | Beaker-Bot | Handles lightweight, isolated experiment runtimes to test novel ideas. |
| `NID-TNK-02` | Bunsen-Bot | Runs performance limits tests, checking system thresholds. |
| `NID-TNK-03` | Pipette-Bot | Collects minute execution metrics from experiment test runs. |
| `NID-TNK-04` | Petri-Bot | Grows mock datasets to support testing operations. |
...
- As an operator, Think Tank has a `docker-compose.production.yml` service with an explicit `PORT`.
...
- As a user, I can exercise: Concept Incubator.
- As a user, I can exercise: Cross-Disciplinary Synthesis.
- As an auditor, every action emits an Observatory event carrying `PID-TNK`.

### Epic 3 — Prove it works

- As a maintainer, `tests/` covers Think Tank's health, status and each ability.
...
**Criticality 7/10 · Readiness 7/10 → Invest — above-median dependency, below-median readiness**
...
| Axis | Score | Reasons |
|---|---|---|
| Criticality | 7/10 | 4 Location(s) name this one's AI as their Prime (+4); answers to 1 Prime(s) (+1); DevOps pillar — platform-wide blast radius (+2) |
| Readiness | 7/10 | status ✅ in CLAUDE.md (+3); code path `src/quantum/` exists on disk (+2); three or more Python files present (+2) |

## 14. Documentation — DERIVED

- `PLATFORM_ENTITIES.md` — canonical entry for PID-TNK
- `src/entities/platform.py` — `PLATFORM_ENTITIES["Think Tank"]`
- `docs/governance/LOCATION-FUNCTIONS.md` — Job Description
- `docs/governance/TRANCENDOS-MODELS-MATRIX.md` — base tier and variants
- `src/quantum/` — implementation
