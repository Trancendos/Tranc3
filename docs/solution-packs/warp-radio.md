...
| Code path | `src/warp_radio/` ✅ on disk | filesystem |
...
- Runs in-process under `api.py` (path `src/warp_radio/`), so it *may*
...
| Readiness | 8/10 | status ✅ in CLAUDE.md (+3); code path `src/warp_radio/` exists on disk (+2); 2 Python file(s) present (+1); compose service `warp-radio` defined (+2) |
