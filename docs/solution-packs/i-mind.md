...
| Code path | `src/imind/` ✅ on disk | filesystem |
...
- Runs in-process under `api.py` (path `src/imind/`), so it *may*
...
| Readiness | 9/10 | status ✅ in CLAUDE.md (+3); code path `src/imind/` exists on disk (+2); 3 Python files present (+2); compose service `imind` defined (+2) |
