...
- **ROUTING DEFECT — `/api/void` has no `stripprefix` and this worker
  does not serve the prefixed path.** Verified against its own source: every
  route it registers sits below `/`, not below `/api/void`, so Traefik
  forwards `/api/void/x` unchanged and the worker 404s on all of them.
...
```
1. Request arrives  →  Traefik forwards /api/void unchanged (no stripprefix)
...
- As a client, requests to `/api/void` reach a route the worker actually serves — today they do not, and this epic is that fix.
- As an implementer, EITHER a stripprefix middleware for `/api/void` is added to the compose labels, OR the worker's router is given the prefix. One of the two, not neither.
