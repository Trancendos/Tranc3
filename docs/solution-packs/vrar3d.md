...
| Code path | `src/vrar3d/` ✅ on disk | filesystem |
...
- Runs in-process under `api.py` (path `src/vrar3d/`), so it *may*
...
```
1. Request arrives  →  Traefik strips /vrar3d
...
- As a client, requests to `/vrar3d` reach the worker with the prefix stripped.
...
| Readiness | 9/10 | status ✅ in CLAUDE.md (+3); code path `src/vrar3d/` exists on disk (+2); 3 Python files present (+2); compose service `vrar3d` defined (+2) |
