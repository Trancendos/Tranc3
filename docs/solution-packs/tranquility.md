---
title: "Solution Pack — Tranquility"
category: Reference
...
| Code path | `src/tranquility/` ✅ on disk | filesystem |
...
- Runs in-process under `api.py` (path `src/tranquility/`), so it *may*
...
**Criticality 7/10 · Readiness 9/10 → Finish first — above-median dependency, above-median readiness**
...
| Readiness | 9/10 | status ✅ in CLAUDE.md (+3); code path `src/tranquility/` exists on disk (+2); 3 Python files present (+2); compose service `tranquility` defined (+2) |
...
- `src/tranquility/` — implementation
