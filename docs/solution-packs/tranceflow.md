...
```
1. Request arrives  →  Traefik forwards /tranceflow unchanged (no stripprefix)
...
- As a client, requests to `/tranceflow` reach the worker with the prefix intact, which is what its own routes expect.
