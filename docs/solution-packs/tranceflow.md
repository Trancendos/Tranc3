---
title: "Solution Pack — TranceFlow"
category: Reference
...
flowchart LR
    C[Client] --> T[Traefik]
    T -->|/tranceflow| S[TranceFlow<br/>no port]
...
| Readiness | 9/10 | status ✅ in CLAUDE.md (+3); code path `src/studio/` exists on disk (+2); 3 Python files present (+2); compose service `tranceflow` defined (+2) |
