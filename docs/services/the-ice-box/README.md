...
| **Foundation** | Cuckoo sandbox (self-hosted) |
...
- **Mission:** sandbox threat isolation & quarantine for untrusted content and code execution.
...
- **Lead AI (Tier 3):** Neonach — role per `PLATFORM_ENTITIES.md`.
...
|---|---|---|---|
| Foundation | Cuckoo sandbox (self-hosted) | self-hosted / OSS | not yet integrated |

No dependency has been added to this repo for The Ice Box; the Foundation column records
platform intent (per `CLAUDE.md`'s Recommended Open Source Foundations table where applicable),
not a committed integration.

## 4. Policy (POL)

- Once implemented, The Ice Box MUST comply with platform-wide policy (`docs/defstan/`,
...
- On implementation, The Ice Box MUST get a full doc-pack upgrade (DDD, TASD, SIM, ASD, PROC, RUN)
...
- Naming: use the canonical name "The Ice Box" exactly as it appears in `CLAUDE.md`'s service table
...
|---|---|---|---|
| 2026-07-04 | Claude (session) | `CLAUDE.md` service table (status, Lead AI, Foundation), `PLATFORM_ENTITIES.md` (identity), repo search confirming no `the_ice_box` implementation exists | Confirmed Planned-tier / no-code status; pack intentionally scoped to GOV+RACI+TFM+POL+STD only per framework §2.1 |
