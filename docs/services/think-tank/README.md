...
| 2026-07-07 | Claude (session, CodeRabbit pass) | `src/quantum/routes.py`, `tests/test_thinktank_routes.py` | Fixed two further CodeRabbit findings. (1) `_quantum_status()`/`_deepmind_status()` re-attempted the full import machinery on every `/thinktank/status` poll — a real cost when the dependency is missing, since Python doesn't cache failed imports in `sys.modules`. Wrapped both in `@lru_cache(maxsize=1)` so availability is resolved once per process; added `cache_clear()` calls in the two status tests so they stay order-independent. (2) `deepmind_plan()`'s error-path response omitted the `depth` field present in the success-path response, giving callers two different response shapes to handle. Added `depth` to the error payload (recomputed defensively, since the failure can occur before `depth` is parsed — falls back to `None` on bad input rather than raising a second exception). Added 2 new tests (`depth` present on a mocked-planner failure; `depth: null` on genuinely unparseable input) — all 6 tests in the file pass; `ruff check` clean. |
| 2026-07-07 | Claude (session, cubic follow-up) | `src/quantum/routes.py`, `tests/test_thinktank_routes.py` | Fixed two further cubic findings on the prior pass's own fixes. (1) `@lru_cache(maxsize=1)` cached the *first* result permanently, including a degraded one — a transient import failure (or polling before a dependency finishes installing) would report "degraded" forever with no way to recover without a process restart. Replaced with a module-level cache that only stores a success result (module import stays valid for the process's lifetime once it succeeds); failures are retried on every call instead of cached, so recovery becomes visible on the next poll. Added a test forcing a fail-then-succeed sequence to confirm recovery. (2) The `depth` recompute in `deepmind_plan()`'s error handler only caught `TypeError`/`ValueError`, so an `OverflowError` (e.g. from `int(float('inf'))`) would propagate past the handler and the sanitized error response would never be returned — added `OverflowError` to the caught exception tuple. All 8 tests in the file pass; `ruff check` clean. |
