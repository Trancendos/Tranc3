...
- **Runtime placement:** three surfaces — a router mounted in the `tranc3-backend` monolith (`src/lab/`) plus **two** separate standalone workers: `the-lab` (port 8055, sandboxed execution) and `lab-service` (port 8066, code generation) — confirmed as two independently-deployed compose services, not one.
- **Persistence:** monolith side shares `tranc3-backend`'s volume; `lab-service` has a named volume; `the-lab` (execution sandbox) has **no** volume.

| Setup | What runs, and where | Data locality | Hard blockers / caveats |
|---|---|---|---|
| **Cloud-Only** | all three surfaces run their own compose block on a single cloud host; Traefik/edge in front | monolith + `lab-service` volume-backed; `the-lab` execution sandbox ephemeral by design (arguably correct for a sandbox) | no entity-specific blocker beyond the sandbox's inherent statelessness |
...
| **Local-Only** | same three surfaces, entirely on local/Citadel hardware | monolith + `lab-service` fully local, volume-backed; `the-lab` sandbox still stateless | none beyond standard local-hardware ops |
