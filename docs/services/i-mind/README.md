...
> (cubic) caught that this pack asserted no implementation exists, when in fact `src/imind/` (`protocol.py` 169 lines, `routes.py`) + `workers/imind/worker.py`
...
|---|---|---|---|
| Foundation | custom (planned `src/imind/`) | self-hosted / OSS | **code exists, integration unverified** — see correction above |

NOTE: this claim is stale — code already exists in this repo for I-Mind (see correction
