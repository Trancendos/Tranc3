...
- **In scope (when built):** the scope implied by the Foundation above; no implementation
  exists yet in this repo, though `src/security/warp_tunnel/` is the planned code path.
...
|---|---|---|---|
| Foundation | custom (planned `src/security/warp_tunnel/`) | self-hosted / OSS | not yet integrated |

No dependency has been added to this repo for The Warp Tunnel; the Foundation column records
...
|---|---|---|---|
| 2026-07-04 | Claude (session) | `CLAUDE.md` service table (status, Lead AI, Foundation), `PLATFORM_ENTITIES.md` (identity), repo search confirming no `src/security/warp_tunnel/` implementation exists | Confirmed Planned-tier / no-code status; pack intentionally scoped to GOV+RACI+TFM+POL+STD only per framework §2.1 |
