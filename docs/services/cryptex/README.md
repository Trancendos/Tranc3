...
|---|---|---|---|
| Foundation | Wazuh + MISP (self-hosted) | self-hosted / OSS | not yet integrated |

No dependency has been added to this repo for Cryptex; the Foundation column records
...
|---|---|---|---|
| 2026-07-04 | Claude (session) | `CLAUDE.md` service table (status, Lead AI, Foundation), `PLATFORM_ENTITIES.md` (identity), repo search confirming no `cryptex` implementation exists | Confirmed Planned-tier / no-code status; pack intentionally scoped to GOV+RACI+TFM+POL+STD only per framework §2.1 |
