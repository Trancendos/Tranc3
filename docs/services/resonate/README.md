...
- **Persistence:** None — this entity is fully stateless (per this pack's own DDD) — no storage of any kind. While the `tranc3-backend` monolith has a named volume, that volume backs *other* entities' state, not this one; this service's own state (if any) is lost on restart/redeploy in every mode alike.
