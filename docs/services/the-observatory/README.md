...
- **Runtime placement:** standalone worker with its own `docker-compose.production.yml` service block (`monitoring`, port 8007) and its own Traefik route — does not run inside the `tranc3-backend` monolith
...
|---|---|---|---|
| **Cloud-Only** | the `monitoring` compose block runs on a single cloud host; Traefik/edge in front | persists via its attached volume as long as the volume/disk is preserved on that host | none beyond standard single-host durability (no built-in cross-host replication) |
...
| **Local-Only** | same `monitoring` compose block, run entirely on local/Citadel hardware behind local Traefik | fully local, volume-backed | none beyond standard local-hardware ops |
