...
- **In scope (when built):** the scope implied by the Foundation above; no implementation
  exists yet in this repo, though `src/studio/` is the planned code path.
...
|---|---|---|---|
| Foundation | custom (planned `src/studio/`) | self-hosted / OSS | not yet integrated |

No dependency has been added to this repo for The Studio; the Foundation column records
...
|---|---|---|---|
| 2026-07-04 | Claude (session) | `CLAUDE.md` service table (status, Lead AI, Foundation), `PLATFORM_ENTITIES.md` (identity), repo search confirming no `src/studio/` implementation exists | Confirmed Planned-tier / no-code status; pack intentionally scoped to GOV+RACI+TFM+POL+STD only per framework §2.1 |
