...
| **Foundation** | Gravitee.io (self-hosted) |
...
> **GOV + RACI + TFM + POL + STD** (intent-level; no DDD/TASD/SIM/ASD/PROC/RUN — no code exists yet for this entity).
> Do not read this pack as describing implemented behaviour.

## 1. Service Governance Charter (GOV)

- **Mission:** central integration hub — REST, webhooks, OAuth exposure of platform APIs to external consumers.
...
- **Lead AI (Tier 3):** Solarscene — role per `PLATFORM_ENTITIES.md`.
- **Owner (RACI-A):** Platform Owner (Trancendos), delegated to Solarscene.
...
| Activity | Platform Owner | Solarscene | Platform Engineering | The Town Hall |
|---|---|---|---|---|
...
|---|---|---|---|
| Foundation | Gravitee.io (self-hosted) | self-hosted / OSS | not yet integrated |

No dependency has been added to this repo for API Marketplace; the Foundation column records
platform intent (per `CLAUDE.md`'s Recommended Open Source Foundations table where applicable),
not a committed integration.

## 4. Policy (POL)

- Once implemented, API Marketplace MUST comply with platform-wide policy (`docs/defstan/`,
...
- On implementation, API Marketplace MUST get a full doc-pack upgrade (DDD, TASD, SIM, ASD, PROC, RUN)
...
- Naming: use the canonical name "API Marketplace" exactly as it appears in `CLAUDE.md`'s service table
...
|---|---|---|---|
| 2026-07-04 | Claude (session) | `CLAUDE.md` service table (status, Lead AI, Foundation), `PLATFORM_ENTITIES.md` (identity), repo search confirming no `api_marketplace` implementation exists | Confirmed Planned-tier / no-code status; pack intentionally scoped to GOV+RACI+TFM+POL+STD only per framework §2.1 |
