...
- **Runtime placement:** standalone worker with its own `docker-compose.production.yml` service block (`library-service`, port 8067) — does not run inside the `tranc3-backend` monolith. **No Traefik label is present on this compose service** — it is currently internal-only, reached by container DNS name + port, not via public routing.
...
|---|---|---|---|
| **Cloud-Only** | the `library-service` compose block runs on a single cloud host, reached internally (no Traefik route) | persists via its attached volume as long as the volume/disk is preserved on that host | none beyond standard single-host durability (no built-in cross-host replication) |
...
| **Local-Only** | same `library-service` compose block, run entirely on local/Citadel hardware, still reached internally (no Traefik route) | fully local, volume-backed | none beyond standard local-hardware ops |
