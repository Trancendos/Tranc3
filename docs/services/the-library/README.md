...
| **Foundation** | Outline (self-hosted, `outline/outline`) |
...
> (cubic) caught that this pack asserted no implementation exists, when in fact `src/library/` + `workers/library-service/worker.py`
...
- **Mission:** knowledge base & wiki for platform documentation, guides, and institutional knowledge.
...
- **Lead AI (Tier 3):** Zimik — role per `PLATFORM_ENTITIES.md`.
- **Owner (RACI-A):** Platform Owner (Trancendos), delegated to Zimik.
...
| Activity | Platform Owner | Zimik | Platform Engineering | The Town Hall |
|---|---|---|---|---|
...
|---|---|---|---|
| Foundation | Outline (self-hosted, `outline/outline`) | self-hosted / OSS | not yet integrated |

No dependency has been added to this repo for The Library; the Foundation column records
platform intent (per `CLAUDE.md`'s Recommended Open Source Foundations table where applicable),
not a committed integration.

## 4. Policy (POL)

- Once implemented, The Library MUST comply with platform-wide policy (`docs/defstan/`,
...
- On implementation, The Library MUST get a full doc-pack upgrade (DDD, TASD, SIM, ASD, PROC, RUN)
...
- Naming: use the canonical name "The Library" exactly as it appears in `CLAUDE.md`'s service table
...
|---|---|---|---|
| 2026-07-04 | Claude (session) | `CLAUDE.md` service table (status, Lead AI, Foundation), `PLATFORM_ENTITIES.md` (identity), repo search confirming no `the_library` implementation exists | Confirmed Planned-tier / no-code status; pack intentionally scoped to GOV+RACI+TFM+POL+STD only per framework §2.1 |
