...
| **Foundation** | custom (planned `src/warp_radio/`) |
...
> (cubic) caught that this pack asserted no implementation exists, when in fact `src/warp_radio/station.py` (143 lines) + `workers/warp-radio/worker.py`
...
- **Mission:** music & audio streaming integration for the platform.
- **In scope (when built):** the scope implied by the Foundation above; no implementation
  exists yet in this repo, though `src/warp_radio/` is the planned code path.
...
|---|---|---|---|
| Foundation | custom (planned `src/warp_radio/`) | self-hosted / OSS | not yet integrated |

No dependency has been added to this repo for Warp Radio; the Foundation column records
platform intent (per `CLAUDE.md`'s Recommended Open Source Foundations table where applicable),
not a committed integration.

## 4. Policy (POL)

- Once implemented, Warp Radio MUST comply with platform-wide policy (`docs/defstan/`,
...
- On implementation, Warp Radio MUST get a full doc-pack upgrade (DDD, TASD, SIM, ASD, PROC, RUN)
...
- Naming: use the canonical name "Warp Radio" exactly as it appears in `CLAUDE.md`'s service table
...
|---|---|---|---|
| 2026-07-04 | Claude (session) | `CLAUDE.md` service table (status, Lead AI, Foundation), `PLATFORM_ENTITIES.md` (identity), repo search confirming no `src/warp_radio/` implementation exists | Confirmed Planned-tier / no-code status; pack intentionally scoped to GOV+RACI+TFM+POL+STD only per framework §2.1 |
