...
| **Foundation** | Paperless-ngx (self-hosted) |
...
- **Mission:** document management hub — ingestion, OCR, storage, and retrieval of platform documents.
...
- **Lead AI (Tier 3):** To be Defined — role per `PLATFORM_ENTITIES.md`.
- **Owner (RACI-A):** Platform Owner (Trancendos), delegated to To be Defined.
...
| Activity | Platform Owner | To be Defined | Platform Engineering | The Town Hall |
|---|---|---|---|---|
...
|---|---|---|---|
| Foundation | Paperless-ngx (self-hosted) | self-hosted / OSS | not yet integrated |

No dependency has been added to this repo for DocUtari; the Foundation column records
platform intent (per `CLAUDE.md`'s Recommended Open Source Foundations table where applicable),
not a committed integration.

## 4. Policy (POL)

- Once implemented, DocUtari MUST comply with platform-wide policy (`docs/defstan/`,
...
- On implementation, DocUtari MUST get a full doc-pack upgrade (DDD, TASD, SIM, ASD, PROC, RUN)
...
- Naming: use the canonical name "DocUtari" exactly as it appears in `CLAUDE.md`'s service table
...
|---|---|---|---|
| 2026-07-04 | Claude (session) | `CLAUDE.md` service table (status, Lead AI, Foundation), `PLATFORM_ENTITIES.md` (identity), repo search confirming no `docutari` implementation exists | Confirmed Planned-tier / no-code status; pack intentionally scoped to GOV+RACI+TFM+POL+STD only per framework §2.1 |
