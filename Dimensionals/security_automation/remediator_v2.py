"""
Dimensionals.security_automation.remediator_v2 — Enhanced auto-remediation engine with rollback.
...
from Dimensionals.security_automation.scanner import (
    Category,
    Violation,
)
...
class FixPathTraversal(ASTSafeTransformer):
    ...
    def fix(self, source: str, violations: List[Violation], filepath: str) -> Tuple[str, List[str]]:
        ...
        lines = source.splitlines()
        fixed_ids = []
        has_import = "from Dimensionals.path_validation import validate_path" in source
        ...
        if fixed_ids and not has_import:
            import_line = "from Dimensionals.path_validation import validate_path"
...
class FixInfoExposure(ASTSafeTransformer):
    ...
    def fix(self, source: str, violations: List[Violation], filepath: str) -> Tuple[str, List[str]]:
        ...
        lines = source.splitlines()
        fixed_ids = []
        has_import = "from Dimensionals.error_handlers import safe_error_detail" in source
        ...
        # Add import if needed
        if fixed_ids and not has_import:
            import_line = "from Dimensionals.error_handlers import safe_error_detail"
            insert_idx = 0
            for i, line in enumerate(lines):
                stripped = line.strip()
                if stripped.startswith("from ") or stripped.startswith("import "):
