"""
Dimensionals.security_automation.remediator — Automated security remediation engine.
...
class AutoRemediator:
    ...
    def _fix_log_injection(self, content: str, violations: List[Violation], filepath: str) -> str:
        ...
        lines = content.split("\n")
        modified = False
        has_sanitize_import = "from Dimensionals.sanitize import sanitize_for_log" in content
    ...
    def _fix_info_exposure(self, content: str, violations: List[Violation], filepath: str) -> str:
        ...
        if (
            ...
            and "from Dimensionals.error_handlers import safe_error_detail" not in content
        ):
            content = self._add_import(
                content,
