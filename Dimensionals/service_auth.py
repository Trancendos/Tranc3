...
into the same trap and stayed unused until two things were fixed:
  ...
  1. Delivery. Own-context workers receive `Dimensionals/` through the
  ...
  2. Import cost. `Dimensionals/__init__.py` used to import the package eagerly,
     which pulled torch and numpy through `genetics` and `liquid`. A worker
     installing five packages could not `from Dimensionals.service_auth import
