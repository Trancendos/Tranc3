# Dimensionals/orchestration/dependency_graph.py
...
class SmartDependencyGraph:
    ...
    def add_edge(
        ...
        if source not in self._nodes or target not in self._nodes:
            logger.warning(
                "Cannot add edge: node not found (%s → %s)",
        ...
        if self._would_create_cycle(source, target):
            logger.warning(
                "Rejecting edge %s → %s: would create cycle",
                sanitize_for_log(source),
                sanitize_for_log(target),
            )
            return None
    ...
    def _compute_impact_level(self, root: str, dependent: str, depth: int) -> ImpactLevel:
        ...
        healthy_alternatives = [
            d
            for d in all_deps_of_dependent
    ...
    def _generate_mitigations(self, node_name: str, impacted: Dict[str, ImpactLevel]) -> List[str]:
        ...
        if dependents and node.health == "unhealthy":
            suggestions.append(f"Consider failing over {node_name} to a healthy replica")
        ...
        depth_2_plus = [
            name for name, level in impacted.items() if level.value >= ImpactLevel.MEDIUM.value
        ]
    ...
    def resilience_score(self, node_name: str) -> float:
        ...
        for dep in deps:
            ...
            alternatives = [
                ...
                if n != dep and nd.node_type == dep_node.node_type and nd.health == "healthy"
            ]
            if alternatives:
                resilient_deps += 1
    ...
    def single_points_of_failure(self) -> List[str]:
        ...
        for node_name in self._nodes:
            ...
            high_plus = sum(
                1 for v in impact.impact_levels.values() if v.value >= ImpactLevel.HIGH.value
            )
