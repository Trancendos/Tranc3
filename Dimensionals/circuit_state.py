...
#     dimensional-nexus-service copy Dimensionals/ subtrees into their build
#     contexts precisely because src/ is unreachable.
#   * `Dimensionals/` is reachable from both sides: services built from the repo
#     root get it on sys.path, and own-context workers vendor it.
#
# So the dependency runs one way — src/ may import Dimensionals, never the
...
# that were sitting inside Dimensionals/ all along, the platform reached eight.
#
