...
## 2024-03-01 - Hardcoded default password hash in api/auth.py
**Vulnerability:** A hardcoded bcrypt hash for the password 'changeme' was used as a fallback for the default 'admin' user if the DEMO_USER_HASH environment variable was missing.
**Learning:** The fallback logic negated the fail-secure principle, creating a predictable attack vector. An unset credential should raise an error or disable the login mechanism completely, similar to what api.py does for the secret key.
**Prevention:** Remove hardcoded fallback credentials and instead raise exceptions or gracefully return None/False when expected environment variables for critical security parameters are missing.
