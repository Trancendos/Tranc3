...
# ── Citadel Instance Configuration ──────────────────────────
# The Citadel is the PRIMARY compute instance for the platform.
# It consumes ALL 4 OCPU and ALL 24 GB RAM of the A1 Flex Always
# Free allocation. Do not run other A1 instances alongside it.

variable "citadel_ocpus" {
  description = "OCPUs for the Citadel instance — must equal the full A1 free-tier allocation (4)"
  type        = number
  default     = 4

  validation {
    condition     = var.citadel_ocpus == 4
    error_message = "Citadel must use exactly 4 OCPUs (the full OCI A1 Always Free allocation)."
  }
}

variable "citadel_memory_gbs" {
  description = "RAM in GBs for the Citadel instance — must equal the full A1 free-tier allocation (24)"
  type        = number
  default     = 24

  validation {
    condition     = var.citadel_memory_gbs == 24
    error_message = "Citadel must use exactly 24 GB RAM (the full OCI A1 Always Free allocation)."
  }
}

variable "citadel_boot_volume_gbs" {
  description = "Boot volume size in GBs for the Citadel instance (47–200 GB; 100 GB recommended)"
  type        = number
  default     = 100

  validation {
    condition     = var.citadel_boot_volume_gbs >= 47 && var.citadel_boot_volume_gbs <= 200
    error_message = "Citadel boot volume must be between 47 and 200 GB."
  }
}

# ── Citadel / Domain Configuration ──────────────────────────

variable "domain" {
  description = "Primary domain for the Trancendos platform (e.g. trancendos.com)"
  type        = string
...
variable "admin_cidr_block" {
  description = "CIDR block permitted to reach admin endpoints (Traefik dashboard :8888). Use your office/home IP with /32."
  type        = string
  default     = "0.0.0.0/0"  # Override in tfvars to restrict access

  validation {
    condition     = can(cidrnetmask(var.admin_cidr_block))
    error_message = "admin_cidr_block must be a valid IPv4 CIDR block (e.g. 203.0.113.10/32)."
  }
}

variable "github_repo_url" {
  description = "Git repository URL to clone onto Citadel during cloud-init bootstrap"
  type        = string
  default     = "https://github.com/Trancendos/Tranc3.git"
}

variable "git_branch" {
  description = "Git branch to check out on Citadel (e.g. main, claude/...)"
  type        = string
  default     = "main"
}
...
variable "cloudflare_zone_id" {
  description = "Cloudflare Zone ID for the domain (found in Cloudflare dashboard → Overview → Zone ID)"
  type        = string
  default     = ""
  sensitive   = true
}
