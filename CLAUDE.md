...
- `docs/governance/DATA-PLATFORM-STRATEGY.md` — the 144 production datastores registered as
  CIs (96 SQLite, 35 Redis, 7 PostgreSQL, 6 DuckDB) and the strategy for moving the
  system-of-record stores off SQLite
- `docs/governance/CONTAINER-JURISDICTION.md` — the 174 containers registered as CIs, each
  carrying a `jurisdiction` (the Location or AI inside it) and a shared `custodian` of
  **The Ice Box**, with a CycloneDX source-scope SBOM per container
- `docs/governance/DOMAIN-MODEL.md` — the Mendix-shaped domain model: Module→Location,
  Module role→Job Description seat, Entity→CI class, and member-level access rules that
  generate both seat permissions and PostgreSQL RLS policies
- `docs/governance/PR-QUEUE-TRIAGE.md` — the pull request queue triaged by what `main` would
  lose rather than by what conflicts, with the merge/close decision and reason for each.
  Generated by `scripts/triage_pull_requests.py`, which exits non-zero when a PR would
  delete a file that is live on `main`
