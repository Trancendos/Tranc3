...
class ConfigDriftDetector:
    ...
    def _diff_env(
        self, baseline: Dict[str, str], current: Dict[str, str]
    ) -> List[DriftItem]:
    ...
    def _diff_params(
        ...
        for key in all_keys:
            ...
            if b_val != c_val:
                severity = DriftSeverity.WARNING if any(
                    p in key.upper() for p in ["PORT", "HOST", "URL"]
    ...
    def _describe_file_drift(key: str, baseline: Dict, current: Dict) -> str:
        ...
        b_status = baseline.get("status", baseline.get("hash", "")[:8] if baseline.get("hash") else "absent")
        c_status = current.get("status", current.get("hash", "")[:8] if current.get("hash") else "absent")
