...
class CircuitBreaker:
    ...
    def _transition_to(self, new_state: CircuitState) -> None:
        ...
        if new_state == CircuitState.OPEN:
            ...
            self._consecutive_successes = 0
            self._half_open_probes = 0
        logger.info("Circuit %s: %s → %s (failures=%d, opens=%d)",
                     sanitize_for_log(self.name), old.value, new_state.value,
                     self._consecutive_failures, self._open_count)
...
class AdaptiveHealthMonitor:
    ...
    def get_all_status(self) -> Dict[str, Any]:
        ...
        for name in self._monitored:
            ...
            circuit = self._circuits.get(name)
            summary[name] = {
                "status": config.get("status", HealthStatus.UNKNOWN).value if isinstance(config.get("status"), HealthStatus) else str(config.get("status", "unknown")),
