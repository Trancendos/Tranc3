"""
shared_core.security_automation.predictor — Violation predictor for proactive security.
...
class PatternRiskDetector:
    ...
    PATTERNS = [
        ...
        (r'logger\.\w+\(f["\']', RiskSignal("CWE-117", "f-string in logger — log injection risk", 0.4)),
        (r'str\(\w+\).*detail', RiskSignal("CWE-209", "str(exc) in detail — info exposure risk", 0.5)),
...
class SafePatternDetector:
    ...
    SAFE_PATTERNS = [
        ...
        (r'from shared_core\.path_validation import', -0.2, "CWE-022"),
        (r'from shared_core\.error_handlers import', -0.2, "CWE-209"),
        (r'from shared_core\.sanitize import', -0.2, "CWE-117"),
    ]
...
class ViolationPredictor:
    ...
    def get_hotspots(self, limit: int = 20) -> List[Prediction]:
        ...
        for filepath, count in sorted(
            self._historical_data.items(), key=lambda x: -x[1]
        )[:limit]:
            if count > 0:
