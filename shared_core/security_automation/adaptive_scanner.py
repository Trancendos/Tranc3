...
class AdaptiveScanner:
    ...
    def suppress(
        ...
        *,
        ...
        entry = SuppressionEntry(
            ...
            reason=reason or f"Suppressed by adaptive scanner",
    ...
    def _context_confidence_modifier(self, v: Violation) -> float:
        ...
        if any(indicator in filepath for indicator in ("_pb2.py", "_grpc.py", ".venv", "node_modules")):
            return 0.3
    ...
    def _load_suppressions(self) -> None:
        ...
        if path.exists():
            try:
                ...
                self._suppressions = [
                    SuppressionEntry(**entry) for entry in data
                ]
    ...
    def _save_history(self) -> None:
        path = Path(self._learning_dir) / _HISTORY_FILE
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(json.dumps(self._history, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
