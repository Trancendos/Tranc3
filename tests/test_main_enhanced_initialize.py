"""TRANC3Enhanced.initialize — subsystem loading and graceful degradation.

Split out of tests/test_main_enhanced.py: three independent contributions
each created that one file with disjoint test sets, so every merge collided
on a file none of them actually shared any test with. Separate modules keep
all the coverage and stop the collision recurring.

This module deliberately does NOT stub torch in sys.modules. The contribution
originally did, at module scope -- which pytest executes at collection, before
any fixture, so it leaked into the whole session and broke an unrelated
isinstance() check in another file. It was also unnecessary:
src/main_enhanced.py guards `import torch` and binds the name at import time,
so a later sys.modules write cannot reach it. Tests needing a stub patch
src.main_enhanced.torch directly instead.
"""
...
async def test_initialize_loads_all_subsystems():
    with patch("src.main_enhanced.logger"):
        ...
        with patch.dict(
            "sys.modules",
            {
                ...
                "src.workflow.executor": MagicMock(
                    event_bus="mock_event_bus", executor="mock_workflow_executor"
                ),
                ...
                "src.healing.self_repair": MagicMock(
                    config_tuner="mock_config_tuner", repair_engine="mock_repair_engine"
                ),
                ...
                "src.core.tranc3_inference": MagicMock(
                    get_engine=MagicMock(
                        return_value=MagicMock(
                            status=MagicMock(return_value={"loaded": True, "device": "cpu"})
                        )
                    )
                ),
                "src.tensorflow_core.hybrid_engine": MagicMock(hybrid_engine="mock_hybrid_engine"),
                "src.bio_neural.consciousness_engine": MagicMock(
                    ConsciousnessModel=MagicMock(return_value="mock_consciousness")
                ),
                "src.evolution.self_improving_core": MagicMock(
                    SelfEvolvingArchitecture=MagicMock(return_value="mock_evolution")
                ),
                "src.holographic.memory_crystal": MagicMock(
                    HolographicMemoryCrystal=MagicMock(return_value="mock_memory")
                ),
                "src.quantum.quantum_core": MagicMock(
                    QuantumNeuralCore=MagicMock(return_value="mock_quantum")
                ),
            },
        ):
    ...
    with patch("src.main_enhanced.logger") as mock_logger:
        ...
        with patch.dict(
            "sys.modules",
            {
                ...
                "src.core.tranc3_inference": MagicMock(
                    get_engine=MagicMock(
                        return_value=MagicMock(status=MagicMock(return_value={"loaded": False}))
                    )
                ),
                "src.tensorflow_core.hybrid_engine": MagicMock(hybrid_engine="mock_hybrid_engine"),
                "src.bio_neural.consciousness_engine": MagicMock(
