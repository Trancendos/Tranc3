...
products_mod = _import_worker("products_service_worker", _TRANC3_ROOT / "workers" / "products-service" / "worker.py")
orders_mod = _import_worker("orders_service_worker", _TRANC3_ROOT / "workers" / "orders-service" / "worker.py")
payments_mod = _import_worker("payments_service_worker", _TRANC3_ROOT / "workers" / "payments-service" / "worker.py")
files_mod = _import_worker("files_service_worker", _TRANC3_ROOT / "workers" / "files-service" / "worker.py")
identity_mod = _import_worker("identity_service_worker", _TRANC3_ROOT / "workers" / "identity-service" / "worker.py")
...
class TestGridHTTPEndpoints:
    ...
    def test_create_workflow(self, grid_client):
        response = grid_client.post("/workflows", json={
            "name": "Test Workflow",
    ...
    def test_execute_workflow(self, grid_client):
        ...
        create = grid_client.post("/workflows", json={
...
class TestGenericCRUDWorker:
    ...
    def test_products_create(self, products_client):
        response = products_client.post("/", json={
            "name": "Test Product",
    ...
    def test_orders_create(self, orders_client):
        response = orders_client.post("/", json={
            "user_id": "user-123",
    ...
    def test_orders_update(self, orders_client):
        create = orders_client.post("/", json={"user_id": "user-123", "total": 10.0, "status": "pending"})
        order_id = create.json()["order_id"]
    ...
    def test_payments_create(self, payments_client):
        response = payments_client.post("/", json={
            "order_id": "order-123",
            "user_id": "user-123",
            "amount": 99.99,
            "currency": "USD",
    ...
    def test_payments_get(self, payments_client):
        create = payments_client.post("/", json={"order_id": "order-123", "user_id": "user-123", "amount": 50.0})
    ...
    def test_payments_update(self, payments_client):
        create = payments_client.post("/", json={"order_id": "order-123", "user_id": "user-123", "amount": 10.0, "status": "pending"})
        payment_id = create.json()["payment_id"]
    ...
    def test_payments_delete(self, payments_client):
        create = payments_client.post("/", json={"order_id": "order-123", "user_id": "user-123", "amount": 5.0})
        payment_id = create.json()["payment_id"]
    ...
    def test_files_create(self, files_client):
        response = files_client.post("/", json={
            "filename": "test.pdf",
            "size_bytes": 1024,
    ...
    def test_files_list(self, files_client):
        files_client.post("/", json={"filename": "file1.txt", "size_bytes": 100, "path": "/files/file1.txt"})
        files_client.post("/", json={"filename": "file2.txt", "size_bytes": 200, "path": "/files/file2.txt"})
    ...
    def test_files_get(self, files_client):
        create = files_client.post("/", json={"filename": "getme.pdf", "size_bytes": 512, "path": "/files/getme.pdf"})
    ...
    def test_files_update(self, files_client):
        create = files_client.post("/", json={"filename": "update.pdf", "size_bytes": 100, "path": "/files/update.pdf"})
        file_id = create.json()["file_id"]
    ...
    def test_files_delete(self, files_client):
        create = files_client.post("/", json={"filename": "delete.pdf", "size_bytes": 50, "path": "/files/delete.pdf"})
        file_id = create.json()["file_id"]
    ...
    def test_identity_create(self, identity_client):
        response = identity_client.post("/", json={
            "user_id": "user-123",
            "provider": "local",
    ...
    def test_identity_list(self, identity_client):
        identity_client.post("/", json={"user_id": "user-1", "provider": "local", "provider_id": "local-1"})
        identity_client.post("/", json={"user_id": "user-2", "provider": "local", "provider_id": "local-2"})
    ...
    def test_identity_get(self, identity_client):
        create = identity_client.post("/", json={"user_id": "user-123", "provider": "local", "provider_id": "local-123"})
    ...
    def test_identity_update(self, identity_client):
        create = identity_client.post("/", json={"user_id": "user-123", "provider": "local", "provider_id": "local-123", "email": "old@example.com"})
        identity_id = create.json()["identity_id"]
