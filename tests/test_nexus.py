"""
Tests for The Nexus — AI, Agent, and Bot Traffic Coordination
=============================================================
...
- Event Router (cross-Nexus event distribution)
- The Nexus (full integration — AI/Agent/Bot traffic)
- FastAPI endpoints
...
class TestNexusIntegration:
    ...
    def setup_method(self):
        self.tmpdir = tempfile.mkdtemp()
        self.db_path = os.path.join(self.tmpdir, "test_nexus.db")
        self.nexus = Nexus(db_path=self.db_path)
...
class TestNexusSingleton:
    ...
    def test_get_nexus_returns_instance(self):
        """get_nexus returns a Nexus instance."""
        nexus = get_nexus()
